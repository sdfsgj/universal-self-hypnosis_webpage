...
(function () {
  ...
  function initSky(canvas) {
    ...
    var flow = null; // constellation light flow state
    var nextAutoFlow = 0;
    ...
    function buildField() {
      ...
      for (i = 0; i < nf; i++) {
        filaments.push({
          ...
          sp: rnd(0.004, 0.011), a: rnd(0.05, 0.13), ph: rnd(0, 6.28)
        });
      }
    }
    ...
    function frame(now) {
      ...
      if (!MOBILE) {
        ...
        px.forEach(function (c, ci) {
          ...
          var pad = 70;
          if (mxi > Math.min.apply(0, xs) - pad && mxi < Math.max.apply(0, xs) + pad &&
              myi > Math.min.apply(0, ys) - pad && myi < Math.max.apply(0, ys) + pad) consActive = ci;
        });
      ...
      px.forEach(function (c, ci) {
        ...
        c.pts.forEach(function (p) {
          ...
          if (p.bright) {
            ...
            ctx.fillStyle = halo;
            ctx.beginPath(); ctx.arc(p.x, p.y, 9, 0, 6.29); ctx.fill();
          }
      ...
      // constellation light flow — a soft light travelling along edges
      if (consActive >= 0 && !REDUCED && introP >= 1) {
        ...
        trail.addColorStop(0, "rgba(236,237,241,0.55)");
        trail.addColorStop(0.4, "rgba(185,192,204,0.18)");
        trail.addColorStop(1, "rgba(185,192,204,0)");
        ctx.fillStyle = trail;
        ctx.beginPath(); ctx.arc(fx2, fy2, 12, 0, 6.29); ctx.fill();
        ctx.fillStyle = "rgba(255,255,255,0.85)";
        ctx.beginPath(); ctx.arc(fx2, fy2, 1.3, 0, 6.29); ctx.fill();
        // destination star softly brightens near the end
        if (flow.p > 0.8) {
          var ba = (flow.p - 0.8) / 0.2;
          var hb = ctx.createRadialGradient(b2.x, b2.y, 0, b2.x, b2.y, 13);
          hb.addColorStop(0, "rgba(236,237,241," + (0.4 * ba).toFixed(3) + ")");
          hb.addColorStop(1, "rgba(236,237,241,0)");
          ctx.fillStyle = hb;
          ctx.beginPath(); ctx.arc(b2.x, b2.y, 13, 0, 6.29); ctx.fill();
        }
      } else { flow = null; }
